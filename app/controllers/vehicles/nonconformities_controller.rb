  class Vehicles::NonconformitiesController < ApplicationController
    ...
    def preprocess_image_params(params)
      [:image1, :image2, :image3, :image4].each do |image_attr|
        if params[:nonconformity][image_attr].present?
          file = params[:nonconformity][image_attr].tempfile
          encoded_data = Base64.strict_encode64(file.read)
          params[:nonconformity][image_attr] = "data:#{params[:nonconformity][image_attr].content_type};base64,#{encoded_data}"
        end
      end
    end
